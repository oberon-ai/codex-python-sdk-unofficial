# ruff: noqa: E402
"""Summarize one UTF-8 text file with the top-level Codex query helper."""

from __future__ import annotations

import argparse
import asyncio
import sys
from pathlib import Path

SRC_ROOT = Path(__file__).resolve().parents[1] / "src"
if str(SRC_ROOT) not in sys.path:
    sys.path.insert(0, str(SRC_ROOT))

from codex_agent_sdk import (
    AgentTextDeltaEvent,
    AppServerConfig,
    CodexOptions,
    TurnCompletedEvent,
    query,
)

DEFAULT_MAX_BYTES = 64 * 1024


class FileInputError(ValueError):
    """Raised when the requested file is unsuitable for this example."""


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("path", help="Path to a UTF-8 text file to summarize.")
    parser.add_argument(
        "--codex-bin",
        help="Optional path to the Codex binary. Defaults to resolving `codex` from PATH.",
    )
    parser.add_argument(
        "--cwd",
        default=str(Path.cwd()),
        help="Workspace directory Codex should use for context. Defaults to the current directory.",
    )
    parser.add_argument(
        "--model",
        help="Optional model override. When omitted, Codex uses its normal default selection.",
    )
    parser.add_argument(
        "--max-bytes",
        type=int,
        default=DEFAULT_MAX_BYTES,
        help=f"Maximum file size in bytes. Defaults to {DEFAULT_MAX_BYTES}.",
    )
    parser.add_argument(
        "--sandbox",
        choices=("read-only", "workspace-write", "danger-full-access"),
        default="read-only",
        help="Sandbox mode for the example. Defaults to read-only for safety.",
    )
    return parser


def load_text_file(path: Path, *, max_bytes: int) -> tuple[Path, bytes, str]:
    resolved = path.expanduser().resolve()

    if not resolved.exists():
        raise FileInputError(f"file does not exist: {resolved}")
    if resolved.is_dir():
        raise FileInputError(f"expected a file, got directory: {resolved}")

    raw_bytes = resolved.read_bytes()
    if len(raw_bytes) > max_bytes:
        raise FileInputError(
            f"file is too large for this example: {len(raw_bytes)} bytes > {max_bytes} bytes"
        )
    if b"\x00" in raw_bytes:
        raise FileInputError(f"refusing binary-looking input: {resolved}")

    try:
        text = raw_bytes.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise FileInputError(f"file is not valid UTF-8: {resolved}") from exc

    return resolved, raw_bytes, text


def _extract_total_tokens(token_usage: object | None) -> int | None:
    total = getattr(token_usage, "total", None)
    total_tokens = getattr(total, "total_tokens", None)
    return total_tokens if isinstance(total_tokens, int) else None


async def run(args: argparse.Namespace) -> int:
    file_path, raw_bytes, file_text = load_text_file(Path(args.path), max_bytes=args.max_bytes)

    prompt = [
        {
            "type": "text",
            "text": (
                "You are reading one UTF-8 text file from disk.\n"
                f"Path: {file_path}\n"
                f"Name: {file_path.name}\n"
                f"Bytes: {len(raw_bytes)}\n\n"
                "Please explain what it does, call out notable details or risks, "
                "and suggest one sensible follow-up question."
            ),
        },
        {
            "type": "text",
            "text": f"File contents:\n```text\n{file_text}\n```",
        },
    ]

    app_server = AppServerConfig(codex_bin=args.codex_bin) if args.codex_bin else None
    options = CodexOptions(
        approval_policy="never",
        cwd=args.cwd,
        model=args.model,
        sandbox_mode=args.sandbox,
    )

    assistant_stream_open = False
    saw_assistant_output = False
    completion_event: TurnCompletedEvent | None = None

    async for event in query(
        prompt=prompt,
        options=options,
        app_server=app_server,
    ):
        if isinstance(event, AgentTextDeltaEvent):
            if not assistant_stream_open:
                print("assistant> ", end="", flush=True)
                assistant_stream_open = True
            saw_assistant_output = True
            print(event.text_delta, end="", flush=True)
            continue

        if isinstance(event, TurnCompletedEvent):
            completion_event = event

    if assistant_stream_open:
        print()

    if (
        completion_event is not None
        and not saw_assistant_output
        and completion_event.result is not None
        and completion_event.result.assistant_text
    ):
        print(f"assistant> {completion_event.result.assistant_text}")

    if completion_event is None:
        print("status: incomplete")
        return 1

    print(f"status: {completion_event.turn_status}")
    total_tokens = _extract_total_tokens(
        None if completion_event.result is None else completion_event.result.token_usage
    )
    if total_tokens is not None:
        print(f"total tokens: {total_tokens}")
    return 0


def main() -> int:
    args = build_parser().parse_args()
    try:
        return asyncio.run(run(args))
    except FileInputError as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1
    except KeyboardInterrupt:
        print("\nInterrupted.", file=sys.stderr)
        return 130
    except Exception as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1


if __name__ == "__main__":
    raise SystemExit(main())
